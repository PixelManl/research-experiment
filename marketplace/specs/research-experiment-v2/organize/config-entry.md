...
```text
src/<package>/config/cli.py   # 仅解析「显式覆盖」；禁止第二套超参源
# 禁止埋在 algorithm/ 训练主循环
```

4. **禁止手写两套 default**（yaml 一份、args.py 再写死一份）。  
   - 唯一默认源 = **yaml**；  
   - 代码侧若要 default，**从 OmegaConf/yaml 读出**，不要复制粘贴数字。  
5. 合并优先级：

```text
yaml 底稿  <  显式 Hydra override（key=value）
禁止：静默的「代码 default 盖过 yaml」
```

6. 未知键：正式模式 **失败**（struct / schema），不要吞掉。  
7. 正式路径不用 Hydra、只丢 `config.snapshot` → **不得**标 runs「当前」（见 [run-evidence.md](./run-evidence.md)）。
